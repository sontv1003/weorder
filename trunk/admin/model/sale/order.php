...
class ModelSaleOrder extends Model {
    ...
    public function editOrder($order_id, $data) {
        ...
        if (isset($data['txt_shipping_fee'])) {
            ...
            $order_shipping = $this->db->query("SELECT order_total_id, value FROM " . DB_PREFIX . "order_total WHERE code = 'shipping_fee' AND order_id = '" . (int) $order_id . "'");
            $fee_order_total_id = null;
            $order_total_fee = 0;

            if ($order_shipping->num_rows) {
                $fee_order_total_id = $order_shipping->row['order_total_id'];
                $order_total_fee = $order_shipping->row['value'];
            }

            if ($order_total_fee == 0)
                $total += $data['txt_shipping_fee'];
            else
                $total = $total + $data['txt_shipping_fee'] - $order_total_fee;
        }

        if (isset($data['txt_pay'])) {

            $order_shipping = $this->db->query("SELECT order_total_id, value FROM " . DB_PREFIX . "order_total WHERE code = 'shipping_pay' AND order_id = '" . (int) $order_id . "'");
            $pay_order_total_id = null;
            $order_total_pay = 0;

            if ($order_shipping->num_rows) {
                $pay_order_total_id = $order_shipping->row['order_total_id'];
                $order_total_pay = $order_shipping->row['value'];
            }

            if ($order_total_pay == 0)
                $total += $data['txt_pay'];
            else
                $total = $total + $data['txt_pay'] - $order_total_pay;
        }
        ...
        if (isset($data['txt_pay'])) {
            $this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET order_total_id = '" . (int) $pay_order_total_id . "', order_id = '" . (int) $order_id . "', code = 'shipping_pay', title = '" . $this->db->escape($data['txt_pay_title']) . "', text = '" . $this->db->escape($data['txt_pay_text']) . "', `value` = '" . (float) $data['txt_pay'] . "', sort_order = '" . (int) $data['txt_pay_sort_order'] . "'");
        }

        if (isset($data['txt_shipping_fee'])) {
            $this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET order_total_id = '" . (int) $fee_order_total_id . "', order_id = '" . (int) $order_id . "', code = 'shipping_fee', title = '" . $this->db->escape($data['txt_shipping_fee_title']) . "', text = '" . $this->db->escape($data['txt_shipping_fee_text']) . "', `value` = '" . (float) $data['txt_shipping_fee'] . "', sort_order = '" . (int) $data['txt_shipping_fee_sort_order'] . "'");
        }
        ...
        $this->db->query("UPDATE `" . DB_PREFIX . "order_total` SET value = '" . (float) $total . "', text = '". $this->currency->format($total, 'VND', 1.00000000) ."' WHERE order_id = '" . (int) $order_id . "' AND code = 'total'");
    }
