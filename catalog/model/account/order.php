...
class ModelAccountOrder extends Model {
    ...
    public function checkOrderStatus($order_id) {
        $query = $this->db->query("SELECT order_status_id  FROM " . DB_PREFIX . "order_history WHERE order_id = '" . (int) $order_id . "' ORDER BY order_history_id DESC LIMIT 0,1");
        return $query->rows;
    }
    
    public function getOrderDetailProducts($order_id) {
        $query = $this->db->query("SELECT op.*, p.image, p.link as main_link FROM " . DB_PREFIX . "order_product op INNER JOIN product p ON p.product_id = op.product_id WHERE op.order_id = '" . (int) $order_id . "' ORDER BY op.link");
    ...
    public function getTotalOrders($order_status_id = null) {
        if (isset($order_status_id)) {
        ...
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE customer_id = '" . (int) $this->customer->getId() . "' AND order_status_id ".$where_order_status_id);
